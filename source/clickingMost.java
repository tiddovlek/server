...
public static int sci[] = {6384,6577,6384,4508,4508,4508,4827,4827,4827,6122,6577};

	public static int sc() {
		return sci[(int) (Math.random() * sci.length)];
	}
	
public void clicking(int a){
client c = (client) server.playerHandler.players[a];

			c.println("ID " + c.actionButtonId);
		...
		switch(c.actionButtonId) {
			case 4169: // Charge arena spells
				...
				if (c.inCombat) {
					c.sM("You're still in combat, which means you cannot log out.");
					break;
}
...
c.sM("You already have the power of vengeance!");
}
} else
...
case 9157:
if(c.NpcDialogue == 1414){
if(c.pkpoints >= 19) {
c.pkpoints -= 19;
c.addItem(sc() ,1);
c.sM("You have recieved an item.");
c.closeAll();
} else {
c.sM("You don't have enough PvPP, you currently have "+c.pkpoints+".");
}
}

if(c.NpcDialogue == 90001){
c.applypvp = 7;
c.closeAll();
c.sM("Your PvP mode will be on in a few seconds...");
}

if(c.NpcDialogue == 2641){
c.closeAll();
c.boss = 0;
c.bossis = 0;
c.rebel = 0;
...
if(c.NpcDialogue == 98789){
c.closeAll();
c.sM("You have accepted the battle!");
c.startfightclan = c.enemyclan;
}

if(c.NpcDialogue == 2640){

if(c.pvpmode == false && c.applypvp == 0) {
...
c.boss = 1;
c.bossis = 1;
c.sM("Your now a boss! Your goal is to kill the rebels!");
} else {
c.closeAll();
c.sM("There is already a boss!");
}

} else {
c.closeAll();
c.sM("You can't be a boss and a rebel!");
}

} else {
c.closeAll();
c.sM("Your PvP mode must be off before you play!");
}

}
if(c.NpcDialogue == 1450){
c.closeAll();
c.duel();
}
if(c.NpcDialogue == 9){
...
if(c.NpcDialogue == 87776){
c.NpcDialogue = 87777;
c.NpcDialogueSend = false;
}
if(c.NpcDialogue == 1337){
c.closeAll();
if(c.pkpoints >= 1) {
c.addItem(553,20);
c.pkpoints -= 1;
c.sM("You brought 20 vengeance skulls. You now have "+c.pkpoints+" points.");
} else {
c.sM("You don't have enough points, you have "+c.pkpoints+" points.");
}
}
...
if(c.NpcDialogue == 2641 || c.NpcDialogue == 87776 || c.NpcDialogue == 1702 || c.NpcDialogue == 11 || c.NpcDialogue == 12 || c.NpcDialogue == 17 || c.NpcDialogue == 24 || c.NpcDialogue == 29 || c.NpcDialogue == 38 || c.NpcDialogue == 43 || c.NpcDialogue == 46 || c.NpcDialogue == 51 || c.NpcDialogue == 58 || c.NpcDialogue == 67 || c.NpcDialogue == 83 || c.NpcDialogue == 93 || c.NpcDialogue == 99 || c.NpcDialogue == 104 || c.NpcDialogue == 110 || c.NpcDialogue == 125 || c.NpcDialogue == 127 || c.NpcDialogue == 129 || c.NpcDialogue == 117){
c.closeAll();
}
if(c.NpcDialogue == 1337){
c.closeAll();
...
c.pkpoints -= 39;
c.gsrights = 1;
c.sM("You can now hold a Godsword! You now have "+c.pkpoints+" points.");
} else {
c.sM("You don't have enough points, you have "+c.pkpoints+" points.");
}
}
...
c.closeAll();
c.rebel = 1;
c.sM("Your now a rebel! Your goal is to kill the boss!");
} else {
c.closeAll();
c.sM("Your PvP mode must be off before you play!");
}
...
if(c.option == 1 && c.pcPoints > 400 && System.currentTimeMillis() - c.lastAction > c.actionInterval){
c.addItem(4151, 1);
c.addItem(4587, 1);
c.pcPoints -= 401;
c.closeInterface();
...
if(c.option == 3 && c.pcPoints < 120 && System.currentTimeMillis() - c.lastAction > c.actionInterval){
c.pcPoints -= 150;
c.closeInterface();
c.actionInterval = 6000;
c.lastAction = System.currentTimeMillis();
...
case 48023://whip
try {
if(c.playerEquipment[c.playerWeapon] == 3098 && c.specialAmount > 49 && c.IsAttacking){
c.specialAtk(true, 50, 1, 2066);
...
if (c.specOn == true && c.playerEquipment[c.playerWeapon] != 4153 && c.playerEquipment[c.playerWeapon] != 3098) {
c.specOn = false;
} else if (c.specOn == false && c.playerEquipment[c.playerWeapon] != 4153 && c.playerEquipment[c.playerWeapon] != 3098) {
c.specOn = true;
}
c.specAttack();
...
case 73100:
if(System.currentTimeMillis() - c.lastAction < c.actionInterval) return;
...
case 2465:
if(c.teleEffect == 1)
c.triggerTele(2580+misc.random(2), 3848+misc.random(2), 0);                                 
break;                        
case 9168:
if(c.teleEffect == 1)
...
case 9190: // FIGHTER MINIGAME TELEPORT(DOOR)

if(c.NpcDialogue == 2537){
c.addSkillXP(4600, 1);
c.closeAll();
}

if(c.NpcDialogue == 7678){
if(c.inCombat) {
c.sM("You cannot teleport in combat!");
break;
}
c.closeAll();
c.triggerTele2(3213, 3423, 0);
}
break;

case 9191: // Paradise Island


if(c.NpcDialogue == 2537){
c.addSkillXP(37224, 1);
c.closeAll();
}
...
c.closeAll();
c.triggerTele2(2964, 3378, 0);
}

if(c.NpcDialogue == 1338){
c.closeAll();
if(c.pkpoints >= 39) {
if(c.clawrights2 == 0) {
c.pkpoints -= 39;
c.clawrights2 = 1;
c.sM("You can now hold black claws! You now have "+c.pkpoints+" points.");
} else {
c.sM("You have already brought the rights to hold this item.");
}
} else {
c.sM("You don't have enough points, you have "+c.pkpoints+" points.");
}
...
if(c.NpcDialogue == 37890){
c.closeAll();
if(c.cwpoint >= 9) {
c.cwpoint -= 9;
c.addItem(6734,1);
c.sM("You brought the fighter torso. You now have "+c.cwpoint+" points.");
} else {
c.sM("You don't have enough points, you have "+c.cwpoint+" points.");
}
}
...
c.pkpoints -= 39;
c.spear = 1;
c.sM("You can now hold a leaf-bladed spear! You now have "+c.pkpoints+" points.");
} else {
c.sM("You have already brought the rights to hold this item.");
}
} else {
...
c.pkpoints -= 39;
c.cutlass = 1;
c.sM("You can now hold a lucky cutlass! You now have "+c.pkpoints+" points.");
} else {
c.sM("You have already brought the rights to hold this item.");
}
} else {
...
c.pkpoints -= 39;
c.clawrights = 1;
c.sM("You can now hold white claws! You now have "+c.pkpoints+" points.");
} else {
c.sM("You have already brought the rights to hold this item.");
}
} else {
...
if(c.NpcDialogue == 2537){
c.addSkillXP(1220000, 1);
c.closeAll();
}

if(c.NpcDialogue == 7678){
c.sM("This area is under construction.");
}
...
c.sM("You now have 99 agility! You now have "+c.pkpoints+" points.");
} else {
c.sM("You already have 99 agility!");
}
} else {
c.sM("You don't have enough points, you have "+c.pkpoints+" points.");
}
...
break;
		...
		case 1526:              
			...
			case 14067:
c.closeInterface();
c.sM("Character designing is currently glitched and is being fixed.");
				break;
