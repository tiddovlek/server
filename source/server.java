...
public static WorldMap WorldMap = null;
	...
	public static int updateSeconds = 10000; // 180 because it doesnt make the
	...
	*/
	public static void calcTime() {
		long curTime = System.currentTimeMillis();
		updateSeconds = 10000 - ((int) (curTime - startTime) / 1800);
		if (updateSeconds == 0) {
			shutdownServer = true;
		}
	}
	...
	public void banHost(String host, int num) {
		if (false) {
			...
			try {
				...
				banned.add(host);
				delay = 2000;
				delayUpdate = System.currentTimeMillis() + 9999;
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
	...
	public void run() {
		...
		try {
			...
			clientListener = new java.net.ServerSocket(serverlistenerPort, 1,
					null);
			misc.println("Crazy PvP V.3 is now online and running!");
			while (true) {
				try {
